...
**Second explicit exception (2026-09-25, owner-approved):** Albert flyer import (`lib/ingestion/albert.ts`). The owner asked for it directly: *"teď bych chtěl naplnit databázi akcemi z Albertu pomocí letáků a OCR přes gemini flash lite, nebo podobný levnější model"*. Albert publishes its prices only as flyer images, and the text layer does not say which price belongs to which product, so the cheapest Gemini Flash-Lite reads each page once (`geminiFlyerExtractor`; the result is cached in `flyer_pages`, never re-read). As with receipts, the model only reads what is printed; a deterministic validator (`validateAlbertOffer`) decides, and accepts an offer only when the page itself confirms it (the price is in the page's text layer, and a printed discount or unit price agrees). This covers that extraction step only — not the AI Shopping Assistant, and not model use anywhere else.

**Third explicit exception (2026-09-26, owner-approved):** Penny flyer import (`lib/ingestion/penny-flyer.ts`, sharing `lib/ingestion/flyer.ts` with Albert). The owner chose it when asked how to read Penny's flyers: *"Jako Albert, přes Gemini"*, and then asked for models from other companies with free limits to be compared. On three identical trial pages Gemini 2.5 Flash without thinking beat Flash-Lite (45 vs 3 validated offers), Mistral Large 3 (32), Mistral Small (24), GPT-4.1 mini (15), Qwen3.7 Flash (14) and Llama 4 (0), so Penny uses `google/gemini-2.5-flash` with thinking off — about $0.16–0.30 per weekly flyer, inside the AI Gateway's monthly free credit. Same rules as Albert: each page is read once and cached in `flyer_pages`; the model only reads; the shared validator decides, and for Penny additionally requires the offer's own unit price to be the one printed with the product's name on the page (`unitPriceOnPage`). This covers that extraction step only — not the AI Shopping Assistant, and not model use anywhere else.
