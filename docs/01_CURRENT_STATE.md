...
* **Search** (`lib/product-search.ts` pure rules, `lib/db/product-search.ts` query, `searchProductsAction`): accent- and case-insensitive substring search over products that have prices ("mleko" finds "Čerstvé mléko"). `products.search_name` (migration `0020`, applied 2026-09-24) is a stored generated column, `lower(translate(name, …))`, using the same character map as `normalizeSearchText()` — a DB test checks the two agree for every mapped character. Words in the query are required; a token with a digit ("1l", "250", "1,5%") is only a rank booster, because names often omit sizes (Lidl's "Mléko polotučné" has none) and requiring them returned nothing. Ranking: whole word > word start > inside a word, a name starting with the first token, an exact name. **Update 2026-09-25, word forms and "is it the product":**
  * Query words are looked up by a stem (`searchStem`: the last vowel dropped for words of 5+ letters), so "Rohlíky" finds "Rohlík" and "Jablka" finds "Jablko".
  * A name word counts as the same word only as the stem plus an inflectional ending (`wordRelation`: exact / form / derived / inside). "Banánové chipsy" and "Rajčatový protlak" are derived words, not bananas or tomatoes.
  * A product is a **direct match** (`isDirectMatch`) only when every query word appears, as the same word, before the first linking word (s, se, z, ze, na, v, ve, do, pro, bez, k, ke, od, po). "Polévka s vejcem" only mentions eggs.
  * Direct matches always rank above mentions. Mentions stay findable in the manual search.
...
Since 2026-09-25 every notification is created through `lib/notify.ts`'s `createHouseholdNotification()` (it also covers the pantry check-in cron, `app/api/cron/pantry-checkin`).

**Push notifications (2026-09-25, code complete; live once configured).** Each member can switch on "Upozornění do telefonu" in the bell panel for the device in their hand; every notification above is then also sent to the household's subscribed phones/browsers as Web Push (details and setup: `docs/push-notifications.md`). The stored notification stays the source of truth, and push is a best-effort copy sent after the response (`after`). A failed push is logged and never fails the action or cron job. The member whose own action raised a notification (expense, list item, joining) is not pushed; the others are. Tapping a notification opens the matching section (`/?tab=…`). Sending uses WebCrypto only (RFC 8291 encryption, RFC 8292 VAPID; `lib/push/web-push.ts`, checked against the RFC's test vector), so it runs on Vercel and on the prepared Cloudflare build without a new dependency. Subscriptions live in `push_subscriptions` (migration 0031), one row per device endpoint. Only endpoints of the known browser push services are accepted (SSRF protection), and rows the push service reports gone (404/410) are deleted. **Off until** migration 0031 is applied and `VAPID_PUBLIC_KEY`/`VAPID_PRIVATE_KEY`/`VAPID_SUBJECT` are set; without them the toggle is hidden and nothing is sent. On iPhone push works only in the app added to the home screen (iOS 16.4+), and the toggle says so.
...
**Second explicit exception (2026-09-25, owner-approved):** Albert flyer offers are read off the flyer page images by Gemini Flash-Lite (`lib/ingestion/albert.ts`) — see `CLAUDE.md` section 30 and section 15's Albert update below. Only that extraction step; everything after it is deterministic.

**Third explicit exception (2026-09-26, owner-approved):** Penny flyer offers are read the same way, by Gemini 2.5 Flash without thinking (`lib/ingestion/penny-flyer.ts`) — see `CLAUDE.md` section 30 and section 15's Penny flyer update below.
...
**Update 2026-09-25, Albert flyer offers read by a model (migration `0029`; owner-approved, `CLAUDE.md` section 30).** Albert has no structured price source (see the 2026-09-23 deep-dive above); its flyers are the only one. `lib/ingestion/albert.ts`: www.albert.cz/aktualni-letaky server-renders the current flyers (validity, `locationType` SUPERMARKET/HYPERMARKET, weekly LEAFLET or themed CATALOG, viewer URL); each flyer's viewer (`letaky.albert.cz/<flyer>/spreads.json`, Publitas) lists the page images and each page's text layer — so no PDF is downloaded. The text layer has names and prices in separate runs, so Gemini Flash-Lite (`google/gemini-2.5-flash-lite` via the AI Gateway) reads each page image with its text layer into structured offers. **Each page is read once:** the result goes to `flyer_pages` (model, token counts), and later runs read it from there; a run sends pages until its time budget minus a minute for writing, and the next run continues. **Nothing the model says is trusted on its own** (`validateAlbertOffer`, pure, with a reason for every rejection): an offer is accepted only when its price is in the page's text layer *and* the page confirms the pairing — the printed discount agrees with the offer and previous price, or the printed unit price agrees with the price and package size. Rejected: Albert-app prices, multi-buy prices, "od" prices, offers with their own validity, non-grocery goods, size ranges ("180–200 g"), pack counts ("5 bal."). The previous price is recorded as the regular price only when the discount confirms it. **Two chains:** Albert's supermarket and hypermarket flyers differ in price, so the SM flyer's deals belong to "Albert" and the HM flyer's to the new chain "Albert Hypermarket" (both chain-wide, `chainWideDeals`); one `albert` source, so a product is one product in both. **Identity:** the flyer has no product id, so the key is brand + name + package size + "vybrané druhy" (`albertProductKey`, diacritics/case/punctuation-insensitive). Cron `/api/cron/ingest-prices/albert` 3× a day (both formats in one run). `pnpm db:albert-flyers [sm|hm] [--pages N] [--apply]` shows offer by offer what was read and why an offer was rejected (dry run writes nothing). **Scale (checked live 2026-09-25):** SM 66 pages (weekly flyer + Bio catalogue), HM 87 pages. **Not verified yet:** the model's accuracy on real pages — the local AI Gateway credentials had expired, so no page has been read by the model yet; run the dry run (`pnpm db:albert-flyers --pages 5`) before relying on the deals. Albert's hypermarket branches are moved to "Albert Hypermarket" by the update below.

**Update 2026-09-26, Penny flyer offers read by a model (migration `0034`; owner-approved, `CLAUDE.md` section 30).** Penny's web shop (`lib/ingestion/penny.ts`, source `penny`) lists only ~40 offers a week; its printed flyer ~400. `lib/ingestion/penny-flyer.ts` (source `penny_flyer`): www.penny.cz/nabidky/letaky links the current flyer as a FlippingBook publication on files.rewe.co.at; each page is its own HTML page whose `#text-container` is the page's text layer, and the page image is `page<nnnn>_4.jpg` (1159 × 2050). **Shared code:** the model step, the page cache, the fetch loop, the validator and the connector moved from `albert.ts` to `lib/ingestion/flyer.ts` (Albert's behaviour unchanged, its tests unchanged); each source only lists its flyers and pages. **Validity:** from the pages' own "Nabídka platná od středy 23. 9. do úterý 29. 9. 2026" (the window most pages print). A page that prints any other date (weekend offers "25. 9. – 27. 9.", a store notice "do 13. 10.") is **not read at all**, because which of its offers hold on other days is visible only on the image; on the first flyer that left out 6 of 37 pages. **Model:** Gemini 2.5 Flash, thinking off (`PENNY_FLYER_MODEL`); Flash-Lite, as at Albert, paired most prices with the wrong product on Penny's dense pages (comparison in `CLAUDE.md` section 30). **Stricter validation for Penny** (`unitPriceOnPage`): Penny prints a unit price in every product's text block ("ŠKVARKOVÝ PAGÁČ* … | 65 g 100 g 10,62 Kč"), so an offer is accepted only when its price ÷ package equals the unit price printed after its name (`unitPriceAfterName`) — a discount alone confirms only that offer and previous price belong together, not to which product. **Display names:** a name printed in capitals is written the catalog way ("Škvarkový pagáč"), footnote stars dropped; the identity key ignores both. Deals belong to the "Penny" chain, chain-wide. Cron `/api/cron/ingest-prices/penny_flyer` at 07:00 and `/penny_flyer/2` at 13:00 UTC (the second continues when the first ran out of time). `pnpm db:penny-flyers [--pages N] [--apply]` shows offer by offer what was read and why an offer was rejected, and which pages were left out (dry run writes nothing). **Verified 2026-09-26 (dry run, whole flyer 23.–29. 9.):** 30 pages read, 496 offers, **227 accepted**, $0.16 of model use. Top rejection reasons: no unit price after the name (57), the price does not fit the unit price printed with the name (76), non-grocery (21), price ranges (20).

**Update 2026-09-25, Albert hypermarkets are their own chain (migration `0030`).** Follows the flyer import above: the hypermarket flyer's deals belong to "Albert Hypermarket", so its branches must be there too. OpenStreetMap does not tell the formats apart, so `lib/stores/albert-formats.ts` asks Albert: its store sitemap names every store page "hypermarket-…" or "supermarket-…", and each hypermarket page server-renders `storeTypeName`, GPS and address (90 hypermarkets, all readable, ~70 s sequentially). `planAlbertFormats()` (pure) matches each to the nearest Albert branch within the OSM import's adoption radius (150 m), or by street and town for a branch without GPS; nothing is created, deleted or moved back, and unmatched hypermarkets are only reported. `syncAlbertStoreFormats()` moves the matched branches in one `db.batch` (one transaction): `store_locations.store_id` plus the branch's `prices`, `purchases` and `receipt_imports`; `member_stores` and `deals` follow by `ON UPDATE CASCADE` on their composite foreign keys (migration `0030`), and a member who had chosen the branch also gets "Albert Hypermarket" selected at chain level. Runs weekly as `/api/cron/albert-store-formats` (Mondays 04:00 UTC, an hour after the OSM import) and at the end of `pnpm db:import-stores`. **Retailer vs. chain** (`lib/stores/chain-family.ts`): outside sources still say "Albert" for a hypermarket, so the OSM import's adoption and a receipt's branch lookup compare the retailer — a moved branch is recognised instead of being added again under "Albert", and a hypermarket receipt is attributed to "Albert Hypermarket". **Checked live (dry run):** of the 23 Albert branches in the database, 6 are hypermarkets (matched at 3–43 m: Ústí n. L. Krušnohorská, Jihlava City Park, Hradec Králové Orlice Park, Liberec Nisa, Pardubice, Jablonec Rýnovka); the other 84 hypermarkets have no branch yet because the OSM branch import has not been applied. The cascade was checked in a rolled-back transaction on the real database. Nothing has been moved yet — the first run after merging does it. The DB-backed receipt tests were not run (no local test database).

**Update 2026-09-25, receipt files on Cloudflare R2 (live for new uploads).** The Vercel Blob store is over its usage limit, so receipt storage now goes through `lib/storage/` (Vercel Blob or R2, chosen for new uploads by `STORAGE_PROVIDER`, default `vercel` = previous behavior). `receipt_imports.image_url` holds a storage reference — a Blob URL for every existing receipt, `r2:receipts/{householdId}/{uuid}.{ext}` for R2 — so each row is read from its own provider and no migration was needed. R2 is used from Vercel over its S3 API (`aws4fetch`), private bucket, keys validated on every access. `scripts/migrate-vercel-blob-to-r2.ts` (`pnpm db:migrate-blob-to-r2`, `--dry-run`, `--rollback <log>`) copies old receipts with a size + SHA-256 check and never deletes Blob files. The app itself stays on Vercel (owner scope decision). Switched on in production the same day (`STORAGE_PROVIDER=r2`); after two fixes (a trailing newline in `R2_BUCKET_NAME`, now trimmed; `Content-Length` lost when Next's `fetch` re-sent a signed `Request` stream, now sent as bytes) the owner confirmed uploads work. Old receipts are not yet copied. The DB-backed receipt tests were not run in the cloud session. Details: `docs/cloudflare-r2.md`, `docs/cloudflare-migration-status.md`.

**Update 2026-09-25, hosting on Cloudflare prepared (PR #83 merged, nothing switched; branch `cloudflare-migration-prep` kept).** At the owner's request the app can now be built and run as a Cloudflare Worker (OpenNext) while Vercel stays production: `open-next.config.ts`, `wrangler.jsonc` (production + staging Worker, 20 Cron Triggers mirroring `vercel.json`), `cloudflare/worker.ts` (OpenNext `fetch` + `scheduled()` calling the cron routes with `CRON_SECRET`), a build-time stand-in for the native `sharp` addon (OCR then gets the original photo — the existing fallback), `lib/gcp-oidc.ts` (PDF OCR can use a self-signed JWT instead of Vercel OIDC; default unchanged), Vercel Analytics off in the Cloudflare build, `.dev.vars.example`. Verified with placeholder secrets: `pnpm cf:build` passes, Worker 4.2 MB gzip (needs Workers Paid), local workerd serves pages, runs `proxy.ts` and the cron chain up to the Neon query; the Vercel build is unchanged. Not verified: anything needing real secrets. Runbook and open risks (experimental Node.js middleware support for `proxy.ts`, OCR without image preparation, CPU limits, origin change): `docs/cloudflare-deployment.md`.

**Update 2026-09-25, section addresses and a remembered dark mode.** The app's sections are no longer only in-memory state: `components/app-shell.tsx` writes the section to the address with `history.pushState` (`/?tab=nakup`, … — `lib/tab-url.ts`; Next 16 integrates native `pushState` with its router) and follows the back/forward buttons, and `app/page.tsx` opens the section named in the address. The AI assistant cannot be opened through `?tab=ai` while `AI_ASSISTANT_ENABLED` is off, and its component no longer shows invented example answers. The dark-mode toggle is stored per device (`lib/theme-preference.ts`); without a stored choice the system setting applies. The theme is applied right after hydration, so a dark-mode phone can see a brief light frame on load.

**Update 2026-09-25, shopping-list comfort.** The list's view (category filter, show completed, sort, grouping) is remembered per device (`lib/list-view-preference.ts`); a "Nechat displej svítit" toggle keeps the phone's screen on while shopping (Screen Wake Lock, `lib/use-wake-lock.ts`, hidden where unsupported); `app/loading.tsx` shows a layout skeleton while the home page renders on the server.
