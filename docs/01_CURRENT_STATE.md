...
* **Not done:** the UI does not show old prices yet (`previousPrice()` is ready for it); the 243 existing official rows keep their wrong 2026-09-19 date (correcting them is a one-off data fix that needs an explicit decision); `deals` still use the app's demo clock for "active"; a same-day price that changes twice keeps only the last value.
