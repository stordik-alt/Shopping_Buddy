...
**First connector done 2026-09-23** (see section 15): Lidl CZ, real pilot batch of 80 grocery products, daily cron. **Second connector done 2026-09-24:** Billa CZ, on a shared connector interface. Still needed: connectors for Penny and the remaining viable sources; a decision on storing Billa promotions that have no end date; widening the Lidl pilot past 80 products once its stability is confirmed over time; flyer-based promotion data (deferred, see section 15).
...
* **Penny** — real prices exist, but no individual product URLs (only ~39 category pages); shares Billa's Nuxt platform, so likely the same extraction approach, but whether one category payload holds its full product list is still unconfirmed.
* **DM drogerie** — not one of the original 6 grocery chains (it's a drugstore, feeding `'Drogerie'` rather than `'Potraviny'`), checked at the owner's request. The cleanest source of this entire research pass: a dedicated 13,047-product sitemap and a clean, unauthenticated REST API (`products.dm.de`) reachable with plain `curl` — real GTIN barcodes, pre-normalized unit prices, fully-populated schema.org markup, no CAPTCHA.
