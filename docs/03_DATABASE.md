...
19. `lib/db/schema.ts` declares every index and constraint the database has (checked against the live catalog 2026-09-25, migration `0032`), including those created by hand-written migrations: `prices_product_context_observed_idx` and `prices_store_location_observed_idx` (latest-observation lookups, `0010`), `store_locations_store_address_city_unique_idx` (one branch per chain and normalized address — receipt imports rely on it, `0011`), and the unique constraint on `invitations.token` under its production name `invitations_token_key`. A new index or constraint is added through the schema and `pnpm db:generate`, not only in a hand-written migration, so the schema stays the complete description.
