...
- ~~stock/storage constraints~~ foundation done: a household pantry (`lib/pantry.ts`, `pantry_items`) tracks what the household believes it has at home, restocked automatically by `completePurchaseAction` and re-checked periodically (per-category interval) by a new daily cron, `/api/cron/pantry-checkin`. The engine itself does not yet consume this to skip already-owned staples when planning a trip — that's the next step; for now it only tracks state and asks the household to confirm/clear it (see `docs/07_CHANGELOG.md`)
