 ...
 *  "s AI" buttons, and the app shell refuses to open its tab (also via `?tab=ai`). Its code stays
 *  as is, except that it no longer answers with invented example data. Switch this on when the AI
 *  phase starts. The receipt OCR import is a separate, owner-approved use of a model and is not
 *  affected. */
